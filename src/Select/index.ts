...
class Select extends FormElement implements ISelect {
	...
	public static defaultOption: SelectOption = {
		useDefaultOptionList: Browser.getBrowser().spec.isTouchable && Browser.getBrowser().spec.ua.iPhone || Browser.getBrowser().spec.ua.iPod || Browser.getBrowser().spec.ua.android,
	};
	...
	protected _createWrapper (): void {
		...
		BaserElement.addClass(this.wrapper, `${Select.classNameSelect}-${FormElement.classNameWrapper}`);
	}
	...
	protected _createPsuedoElements (): void {
		...
		BaserElement.addClass(this.$pseudo[0], FormElement.classNameFormElementCommon);
		BaserElement.addClass(this.$pseudo[0], Select.classNamePseudoSelect);

		this.$selected = $('<span />');
		this.$selected.appendTo(this.$pseudo);
		BaserElement.addClass(this.$selected[0], FormElement.classNameFormElementCommon);
		BaserElement.addClass(this.$selected[0], Select.classNamePseudoSelect, Select.classNamePseudoSelectedDisplay);

		if (!this._config.useDefaultOptionList) {
			this.$options = $('<ul />');
			this.$options.appendTo(this.$pseudo);
			BaserElement.addClass(this.$options[0], FormElement.classNameFormElementCommon);
			BaserElement.addClass(this.$options[0], Select.classNamePseudoSelect, Select.classNameSelectOptionList);
			$(this.el).find('option').each( (i: number, opt: HTMLElement): void => {
				...
				$psuedoOpt.text(text);
				BaserElement.addClass($psuedoOpt[0], FormElement.classNameFormElementCommon);
				BaserElement.addClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption);
			});
		}

		if (Browser.getBrowser().spec.isTouchable) {
			if (Browser.getBrowser().spec.ua.iPhone || Browser.getBrowser().spec.ua.iPod) {
				this.addClass(Select.classNameOsIOs);
				BaserElement.addClass(this.wrapper, Select.classNameOsIOs);
				BaserElement.addClass(this.label, Select.classNameOsIOs);
			} else if (Browser.getBrowser().spec.ua.android) {
				this.addClass(Select.classNameOsAndroid);
				BaserElement.addClass(this.wrapper, Select.classNameOsAndroid);
				BaserElement.addClass(this.label, Select.classNameOsAndroid);
			}
		}

		if (this._config.useDefaultOptionList) {
			this.addClass(Select.classNameUseDefaultOptionList);
			BaserElement.addClass(this.wrapper, Select.classNameUseDefaultOptionList);
			BaserElement.addClass(this.label, Select.classNameUseDefaultOptionList);
		}
	...
	protected _onfocus (): void {
		if (!this.hasFocus) {
			...
			super._onfocus();
			// DOMのclassを制御
			BaserElement.addClass(this.$pseudo[0], Select.classNamePseudoSelect, '', FormElement.classNameStateFocus);
			BaserElement.removeClass(this.$pseudo[0], Select.classNamePseudoSelect, '', FormElement.classNameStateBlur);
			// スクロール位置を調整する
	...
	protected _onblur (): void {
		...
		if (this.$pseudo) {
			super._onblur();
			BaserElement.addClass(this.$pseudo[0], Select.classNamePseudoSelect, '', FormElement.classNameStateBlur);
			BaserElement.removeClass(this.$pseudo[0], Select.classNamePseudoSelect, '', FormElement.classNameStateFocus);
		}
	}
	...
	private _update (): void {
		...
		$(this.el).find('option').each( (i: number, opt: HTMLElement): void => {
			...
			if (this.$options) {
				...
				if (isSelected) {
					BaserElement.addClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateSelected);
					BaserElement.removeClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateUnselected);
				} else {
					BaserElement.addClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateUnselected);
					BaserElement.removeClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateSelected);
				}
				if (isDisabled) {
					BaserElement.addClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateDisabled);
				} else {
					BaserElement.removeClass($psuedoOpt[0], Select.classNameSelectOptionList, Select.classNameSelectOption, Select.classNameStateDisabled);
				}
			}
		});
	...
	private _scrollToSelectedPosition (): void {
		if (this.$options) {
			let $selectedPsuedoOpt: JQuery | undefined;
			...
			const optPos: JQueryCoordinates = $selectedPsuedoOpt ? $selectedPsuedoOpt.offset() : { top: 0, left: 0 };
